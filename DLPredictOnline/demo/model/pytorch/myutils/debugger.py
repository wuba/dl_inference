...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif num_classes == 22 or dataset == "watermark":
        self.names = watermark_class_name
  ...
  def save_img(self, imgId='default', path='./cache/debug/', image_name = 'default'):
    cv2.imwrite(path + '{}'.format(image_name), self.imgs[imgId])
  ...
  def save_all_imgs_test(self, path='./cache/debug/', prefix='', genID=False, image_name= 'image'):
    if genID:
      try:
        idx = int(np.loadtxt(path + '/id.txt'))
      except:
      ...
      np.savetxt(path + '/id.txt', np.ones(1) * (idx + 1), fmt='%d')
    for i, v in self.imgs.items():
      cv2.imwrite(path + '/{}{}.png'.format(image_name, i), v)
...
watermark_class_name = [
    "365Taofang", "Fangtianxia",
    "Fangchanchaoshi", "Beikezhaofang",
    "Dazhongdianping", "Xianyu",
    "58tongcheng", "Anjuke",
    "Kujiale", "Soufang",
    "Qichezhijia", "Renrenche",
    "Souhuershou", "Youxin",
    "Baixingwang", "Diyichewang",
    "Beikezhaofang1", "Lianjia",
    "Lianjia1", "Xinlangershoufang",
    "Tubatu", "Loupanwang"]
