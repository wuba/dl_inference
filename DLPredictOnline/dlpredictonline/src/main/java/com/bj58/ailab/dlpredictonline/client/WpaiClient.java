...
import com.bj58.ailab.dlpredictonline.entity.PredictionProtos.RequestResponse;
...
//        pytorchClient();
    ...
    public void tensorflowClient(){
        ...
        TensorflowClient tensorflowClient = new TensorflowClient();
        PredictRequest request = tensorflowClient.getRequest();
        Predict.PredictResponse response = null;
        try {
            //使用阻塞 stub调用
            response = blockingStub.predict(request);
            tensorflowClient.printResult(response);
        } catch (Exception e) {
